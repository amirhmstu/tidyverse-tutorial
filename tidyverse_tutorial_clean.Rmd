---
title: "Analyzing Psychology Data in the Tidyverse"
author: "Mike Frank"
date: "10/1/2018"
...
```{r}
mean(unique(sort(iris$Petal.Length)))
...
sgf[sgf$age >= 2 & sgf$age < 3 & sgf$condition == "Label", ]

with(sgf, sgf[age >= 2 & age < 3 & condition == "Label", ])

```


(Note you can give two conditions, could also do `age > 2 & age < 3`). (equivalent: `filter(sgf, age > 2, age < 3)`)
...
**Exercise.** Create a smaller datast with **only** the "face" trial in the "Label" condition.
...
# for (i in 1:length(unique(sgf$subid))) {
#   for (j in 1:length(unique(sgf$condition))) {
#     this_data <- sgf[sgf$subid == unique(sgf$subid)[i] & 
...
```{r}
ggplot(sgf_means, 
       aes(x = age_group, y = correct, col = condition, 
           group = condition)) + 
  geom_line() + 
  ylim(0,1) 
```
...
sgf_sub_means <- sgf %>%
  ...
  summarise(sub_mean = mean(correct), 
            sub_sd = sd(correct))

sgf_grand_means <- sgf_sub_means %>%
  group_by(condition) %>%
  summarise(condition_mean = mean(sub_mean), 
            condition_sd = sd(sub_mean), 
            n = n())

# tricky but efficient
sgf_grand_means2 <- sgf %>%
...
```{r}
sgf_wide %>% 
  gather(item, correct, 6:9)

sgf_wide %>% 
  gather(item, correct, faces:pasta)

sgf_wide %>% 
  gather(item, correct, starts_with("be"))
```
